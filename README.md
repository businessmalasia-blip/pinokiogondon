...
3. **Три фильтра** (`bot/filters.py`):
   - `check_concentration` — крупнейшие держатели через
     `getTokenLargestAccounts` (текущее состояние сети, без задержки
     DAS-индексации); supply берётся из аккаунта bonding curve; кривая
     исключается по адресу; ни один холдер не держит > 2.5% supply,
     топ-10 в сумме < 15%.
