...
3. **Три фильтра** (`bot/filters.py`):
   ...
   - `calculate_human_percent` — HUMAN-кошельки (есть история транзакций,
     ...
     (по умолчанию ≥ 45% / ≤ 30%).
   - бандл-метрика — доля транзакций токена, севших пачками в один слот
     (считается по тем же сигнатурам минта, что грузит `check_dev`, без
     дополнительных запросов); порог `MAX_BUNDLE_PERCENT` (по умолчанию 25%).
...
4. **Скоринг** (`bot/scoring.py`) — по посчитанным метрикам (HUMAN %, MSR,
   топ-10 концентрация, % бандлов) выставляются баллы 0–10, взвешенная сумма
   (веса из `.env`: HUMAN 30%, MSR 25%, концентрация 25%, бандлы 20%) даёт
   итоговый скор; алерт отправляется только при скоре ≥ `MIN_SCORE`
   (по умолчанию 7.5).
